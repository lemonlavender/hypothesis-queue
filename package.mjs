...
export function buildPackages(root, out) {
  ...
  if (manifest.id !== 'hypothesis-queue' || !/^\d+\.\d+\.\d+(?:-[a-zA-Z0-9.-]+)?$/.test(manifest.package.version)) throw new Error('Unexpected package identity/version');
  ...
  const stage = mkdtempSync(path.join(tmpdir(),'hypothesis-package-'));
